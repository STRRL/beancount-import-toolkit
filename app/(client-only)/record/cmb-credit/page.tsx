...
export default function CMBCreditPage() {
    function cutStringBoundaryExcluded(
        str: string,
        start: string,
        end: string
    ): string {
    ...
    const [accountName, setAccountName, purgeAccountName] = useLocalStorage(
        "beancount-import-toolkit.cmb-credit-account-name",
        "Assets:Bank:CMBC:CreditCard"
    );
    const [year, setYear, purgeYear] = useLocalStorage(
        "beancount-import-toolkit.year-of-the-statements",
        "2023"
    );

    const [rawText, setRawText] = useState("");

    const [rulesOrUndefined, setRules, purgeRules] = useLocalStorage(
        "beancount-import-toolkit.cmb-credit-rules-json",
        [] as TransformRule[]
    );
    ...
    const [rulesModalMode, setRulesModalMode] = useState<
    ...
    const beancountTxns = useMemo(() => {
        ...
        return rawTxns.map((it) =>
            cmbCreditRawTxn2BeancountTxn(it, accountName || "", year || "2023")
        );
    ...
    const loadFile = async (e: ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            ...
            if (file.name.endsWith(".pdf")) {
                ...
                let tablePart = cutStringBoundaryExcluded(
                    content,
            ...
            <div className="container mx-auto h-[80vh] p-4">
                <div className="flex items-center pb-4">
                    ...
                    <div>
                        <input
                            type="file"
                            className="file-input file-input-sm file-input-primary w-full max-w-xs"
                            onChange={(e) => {
                                loadFile(e);
                            }}
                        />
                            ...
                            <div className="flex-1 w-full flex flex-col h-[35vh] overflow-y-auto">
                                <div className="w-full pt-2 grid grid-cols-2 gap-6">
                                    {rules.map((rule, index) => {
                                        return (
                                            <div className="card bg-base-200" key={`rule-${index}`}>
                                                <div className="card-body py-4">
                                                    ...
                                                    <div>
                                                        {rule.criteria.map((c, i) => {
                                                            return (
                                                                <div
                                                                    key={`rule-${index}-criterion-${i}`}
                                                                    className="flex"
                                                                >
                                                                    <h2 className="card-title flex-1 text-sm">
                                                                        <span>
                                                                            field{" "}
                                                                            <a className="text-primary">{`"${c.field}"`}</a>{" "}
                                                                            {c.operator}{" "}
                                                                            <a className="text-primary">{`"${c.value}"`}</a>
                                                                        </span>
                                                                    </h2>
                                                                </div>
                                                            );
                                                                ...
                                                                >
                                                                    {a.setPayee != undefined ? (
                                                                        <h2 className="card-title flex-1 text-sm">
                                                                            <span>
                                                                                set payee to{" "}
                                                                                <a className="text-primary">{`"${a.setPayee}"`}</a>
                                                                            </span>
                                                                        </h2>
                                                                    ) : (
                                                                        ""
                                                                    ...
                                                                    {a.setNarration != undefined ? (
                                                                        <h2 className="card-title flex-1 text-sm">
                                                                            <span>
                                                                                set narration to{" "}
                                                                                <a className="text-primary">{`"${a.setNarration}"`}</a>
                                                                            </span>
                                                                        </h2>
                                                                    ) : (
                                                                        ""
                                                                    ...
                                                                    {a.setTargetAccount != undefined ? (
                                                                        <h2 className="card-title flex-1 text-sm">
                                                                            <span>
                                                                                set target account to{" "}
                                                                                <a className="text-primary">{`"${a.setTargetAccount}"`}</a>
                                                                            </span>
                                                                        </h2>
                                                                    ) : (
                                                                        ""
                                                    ...
                                                    <div className="flex">
                                                        <button
                                                            className="flex-1 btn btn-xs mx-2"
                                                            onClick={() => {
